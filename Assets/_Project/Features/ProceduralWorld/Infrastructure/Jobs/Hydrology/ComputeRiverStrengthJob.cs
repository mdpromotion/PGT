...
namespace _Project.Features.ProceduralWorld.Infrastructure.Jobs.Hydrology
{
    ...
    public struct ComputeRiverStrengthJob : IJobParallelFor
    {
        ...
        private float SampleMacroBilinear(float2 worldPos)
        {
            ...
            float h00 = MacroAccumulation[z0 * MacroPaddedSize + x0];
            float h10 = MacroAccumulation[z0 * MacroPaddedSize + x1];
            float h01 = MacroAccumulation[z1 * MacroPaddedSize + x0];
            float h11 = MacroAccumulation[z1 * MacroPaddedSize + x1];
